...
const LessonPage = () => {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        ...
        const data2 = await fetch(baseUrl + `/lesson/get-lesson/${id}`).then(
          (res) => res.json(),
        );
  ...
  if (!user) {
    return (
      <div className="flexCenter h-[calc(100vh-250px)] w-full flex-col gap-5 font-bold">
  ...
  if (
    ...
    return (
      <div className="flexCenter h-[calc(100vh-250px)] w-full font-bold">
  ...
  return (
    <>
      <Head>
        <title>{lesson?.title || "Lesson Page"}</title>
      </Head>
      {isLoading ? (
        ...
        <>
          {lesson && isBrowser && (
            <>
              ...
              <div className="paddings flex w-full flex-row justify-between ">
                <h1 className="flex flex-row items-center gap-x-3 text-2xl font-bold">
                  <span>{lesson.title}</span>
                  {renderDoneIcon(lesson._id)}
                </h1>
                <p className="text-start">Duration: {lesson.duration}min</p>
              </div>
              <div className="flexCenter h-full w-full flex-col bg-base-300">
                <div className="flex w-full flex-col justify-start p-5">
                  <h3 className="text-2xl">Description: </h3>
                  <div className="my-3 w-full rounded-lg bg-base-200 p-3 ">
                    <div
                ...
                <div className="mb-16 flex w-full flex-row justify-around">
                  <div>
                    <button
