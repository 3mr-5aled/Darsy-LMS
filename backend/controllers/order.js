...
const getSingleOrder = asynchandler(async(req,res,next)=>{
  const order = req.user.role ==="tutor" ? await Order.findById(req.params.orderId).populate([{path:'userId',select:'name'},{path:'adminId',select:'name'},{path:'courseId',select:'name'}]).sort("-createdAt") :await Order.findById(req.params.orderId).populate({path:"userId",select:'name'}).select(' -adminId -courseId -tran_ref ').sort("-createdAt")
  if(!order){
...
const getAllOrdersByDay = asynchandler(async(req,res,next)=>{
  ...
  await Order.find({ userId:req.user._id, createdAt: { $gte: startDate, $lt: endDate }}).populate({path:"userId",select:'name'}).select(' -adminId -courseId -tran_ref ').sort("-createdAt")
  ...
  await Order.find({ $or: [{ userId:req.user._id,createdAt: { $lt: startDate } }, { userId:req.user._id, createdAt: { $gt: endDate } }] }).populate({path:"userId",select:'name'}).select(' -adminId -courseId -tran_ref ').sort("-createdAt");
